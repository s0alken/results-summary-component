...
const App = () => {
  ...
  useEffect(() => {
    animate(numerito, 67, {
      delay: 1, duration: 1, onUpdate: latest => {
        setNumerito(Math.floor(latest))
      }
    })
  }, []) */
      ...
      >
        ...
        >
          ...
          <div className='grid gap-[8px] md:gap-[14px] md:max-w-[260px] md:mx-auto'>
            <motion.h2 variants={transitionVariant} className='font-bold text-white text-[24px] leading-[31px] md:text-[32px] md:leading-[42px]'>Great</motion.h2>
            <motion.p variants={transitionVariant} className='font-medium text-light-lavender text-[16px] leading-[21px] md:text-[18px] md:leading-[23px]'>
        ...
        >
          <motion.h1 variants={transitionVariant} className='text-dark-gray-blue font-bold text-[18px] leading-[23px] md:text-[24px] md:leading-[31px] md:mb-[28px]'>
            Summary
          </motion.h1>

          <div className='grid gap-[16px] text-[16px] leading-[21px] md:mb-[41px] md:text-[18px] md:leading-[23px]'>
            {data.map(item => <SummaryItem key={item.category} item={item} variants={transitionVariant} />)}
          </div>
